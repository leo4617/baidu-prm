...
$stk = 'stok/';   // 临时缓存目录
$ct  = 604800;     // 缓存时间 以秒数计算 如 1分钟=60 1天=86400 1周=604800 30天=2592000
...
';
echo '<link href="http://'.$_SERVER['HTTP_HOST'].$u;
if (strlen(@$x) > 0) {
    echo $l;
}
...
if (strlen($s) == 0) {
    ...
    curl_setopt($c, CURLOPT_TIMEOUT, 3);
    curl_setopt($c, CURLOPT_URL, 'http://entry.baidu.com/rp/api?di=api100008&qnum=40');
    $sohu = json_decode(curl_exec($c), 1);
    curl_close($c);
    if ($sohu['st'] > 0) {
        echo '
    <table>
        <tbody class="center">';
        shuffle($sohu['data']['recommends']);
        foreach ($sohu['data']['recommends'] as $i => $v) {
            if ($sohu['data']['recommends'][$i]['type'] == 141) {
                $h_wd[$i] = strtolower($sohu['data']['recommends'][$i]['word']);
                if ($i % 4 == 0) {
                    echo '<tr>';
                }
                echo '
            <td><a itemprop="url" href="'.$u.$l.preg_replace('/(\s+)/', '%20', $h_wd[$i]).'" target="_blank">'.$h_wd[$i].'</a></td>';
                $i++;
                if ($i % 4 == 0) {
                    echo '
            </tr>';
...
if (strlen($s) > 0) {
    ...
    <div class="soft break mb" style="padding:0 1em 0 1em;" itemprop="articleBody">
        ...
        <p>';
                    ...
                    echo '</p>
        <p style="text-align:right">小编&nbsp;罗罗</p>
    </div>';
                }
            }
        }
